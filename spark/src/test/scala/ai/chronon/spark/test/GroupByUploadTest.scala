...
class GroupByUploadTest {
  ...
  def incrementalSnapshotEventsUploadMatchesNonIncremental(): Unit = {
    ...
    rows
      .toDF("user", "price", "ts", partitionCol)
      .save(s"$namespace.$eventsTable", partitionColumns = Seq(partitionCol))
    ...
    def conf(name: String, incremental: Boolean): GroupBy = {
      val gb = Builders.GroupBy(
        sources = Seq(
          Builders.Source.events(Builders.Query(selects = Builders.Selects("user", "price", "ts")),
                                 table = eventsTable)),
        keyColumns = Seq("user"),
        aggregations = aggregations,
