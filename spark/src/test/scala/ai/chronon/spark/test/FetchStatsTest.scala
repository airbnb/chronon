...
class FetchStatsTest extends TestCase {
  ...
  def testFetchStats(): Unit = {
    ...
    val queriesDf = tableUtils.sql(s"SELECT ts FROM ${joinConf.metaData.dailyStatsOutputTable} WHERE ds = '$yesterday'")
    queriesDf.show()
    def buildRequests(): Array[Request] =
      queriesDf.rdd
        .map { row =>
          val keyMap = Seq("ts" -> row.getLong(0).asInstanceOf[AnyRef]).toMap
