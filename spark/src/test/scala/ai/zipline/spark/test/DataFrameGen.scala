...
object DataFrameGen {
  ...
  def mutations(spark: SparkSession, columns: Seq[Column], count: Int, partitions: Int, mutationProbability: Double,
    ...
    val withInserts = generated
      ...
      .withColumn(Constants.PartitionColumn, from_unixtime((generated.col(Constants.TimeColumn) / 1000), Constants.Partition.format))
      .drop()
    ...
    val mutatedFromDf = withInserts
      ...
      .withColumn(Constants.MutationTimeColumn, randomLerp(col(Constants.MutationTimeColumn), unix_timestamp(col(Constants.PartitionColumn), Constants.Partition.format) * 1000 + 86400 * 1000))
    ...
    val realizedTo = spark.createDataFrame(spark.sparkContext.parallelize(mutatedToDf.rdd.collect()), mutatedToDf.schema)
    ...
    val expandedEventsDf = seedDf.as("sd")
      .join(
