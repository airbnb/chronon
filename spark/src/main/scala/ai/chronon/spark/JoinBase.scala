...
abstract class JoinBase(joinConf: api.Join,
  ...
  def computeLeft(overrideStartPartition: Option[String] = None): Unit = {
    ...
    if (unfilledRanges.isEmpty) {
      ...
      val leftSchema = leftDf(joinConf, unfilledRanges.head, tableUtils, allowEmpty = true, limit = Some(0)).map(df => df.schema)
  ...
  def computeJoinOpt(stepDays: Option[Int] = None,
    ...
    val leftSchema =
      leftDf(joinConf, PartitionRange(endPartition, endPartition)(tableUtils), tableUtils, allowEmpty = true, limit = Some(0)).map(df =>
        df.schema)
