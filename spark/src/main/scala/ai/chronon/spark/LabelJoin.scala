...
class LabelJoin(joinConf: api.Join, startOffset:Int, endOffset:Int, tableUtils: TableUtils) {
  ...
  assert(startOffset >= endOffset,
    s"Start time offset ${startOffset} must be earlier than end offset ${endOffset}")
  ...
  def computeLabelJoin(stepDays: Option[Int] = None): DataFrame = {
    ...
    val labelJoinLeft = joinConf.left.updateQueryPartitions(leftStart, leftEnd)
