...
# Chronon Developer Setup Script
# Full production-like setup for advanced development
# Includes: Spark + Iceberg + MongoDB + MinIO + Airflow + DynamoDB Local
...
COMPOSE_FILE="docker-compose-developer.yml"
MINIO_ALIAS="local"
MINIO_ENDPOINT="http://localhost:9000"
MINIO_ACCESS_KEY="minioadmin"
MINIO_SECRET_KEY="minioadmin"

echo "👨‍💻 Starting Chronon Developer (full setup)"
...
check_prerequisites() {
    ...
    local ports=(9000 9001 27017 8080 7077 8888 8085 8000)
...
start_core_services() {
    ...
    # Developer: Start all services including Airflow and DynamoDB
    docker-compose -f "$COMPOSE_FILE" up -d minio mongodb airflow-db dynamodb-local
    ...
    wait_for_service "Airflow DB" "docker-compose -f $COMPOSE_FILE exec airflow-db pg_isready -U airflow"
    
    # DynamoDB Local doesn't have health check, just wait a bit for it to start
...
start_processing_services() {
    ...
    # Developer: Start Airflow (for production-like orchestration)
    print_status "Starting Airflow services (production-like orchestration)..."
    docker-compose -f "$COMPOSE_FILE" up -d airflow-scheduler airflow-webserver
    
    # Give Airflow time to initialize (don't wait for it to be ready)
    print_status "Airflow services starting in background (may take time to initialize)..."
...
# Function to setup Airflow (production-like orchestration)
setup_airflow() {
    print_status "Setting up Airflow for production-like Spark job submission..."
    
    # Create Airflow directories
    mkdir -p airflow/dags airflow/logs airflow/plugins
    
    # Wait for Airflow services to be ready
    print_status "Waiting for Airflow services to initialize..."
    sleep 30
    
    # Initialize Airflow database
    print_status "Initializing Airflow database..."
    if docker-compose -f "$COMPOSE_FILE" exec airflow-webserver airflow db init 2>/dev/null; then
        print_success "Airflow database initialized successfully"
        
        # Create admin user
        print_status "Creating Airflow admin user..."
        if docker-compose -f "$COMPOSE_FILE" exec airflow-webserver airflow users create \
            --username admin \
            --firstname Admin \
            --lastname User \
            --role Admin \
            ...
            print_warning "Airflow admin user creation failed - you can create it manually later"
        fi
        
        # Test Airflow connectivity
        if docker-compose -f "$COMPOSE_FILE" exec airflow-webserver airflow dags list >/dev/null 2>&1; then
            print_success "Airflow is ready for Spark job submission!"
        else
            print_warning "Airflow may still be initializing - check http://localhost:8085"
        fi
        
    else
        print_warning "Airflow database initialization failed"
        print_status "This may be due to PostgreSQL compatibility issues"
        print_status "You can still use Chronon GroupBy features with direct run.py commands"
        print_status "To troubleshoot Airflow: docker-compose -f $COMPOSE_FILE logs airflow-webserver"
    fi
...
verify_services() {
    ...
    # Test Airflow (separate check since it can take time to initialize)
    if docker-compose -f "$COMPOSE_FILE" ps airflow-webserver | grep -q "Up"; then
        print_success "Airflow webserver container is running"
        # Try to access the web interface
        if curl -f http://localhost:8085 >/dev/null 2>&1; then
            print_success "Airflow web interface is accessible at http://localhost:8085"
        else
            print_warning "Airflow web interface may still be initializing"
        fi
    else
        print_warning "Airflow webserver may not be fully ready"
    fi
    
    # Test DynamoDB Local (separate check since it doesn't have curl)
    if docker-compose -f "$COMPOSE_FILE" ps dynamodb-local | grep -q "Up"; then
        print_success "DynamoDB Local container is running"
    else
        print_warning "DynamoDB Local may not be fully ready"
    fi
...
display_access_info() {
    print_success "Chronon Developer Setup Completed!"
    echo
    ...
    echo "Airflow Dashboard: http://localhost:8085 (admin/admin) - PRODUCTION-LIKE Spark job submission"
    echo
    echo "=== Next Steps ==="
    echo "👨‍💻 Follow the developer guide: CHRONON_DEVELOPER_GUIDE.md"
    echo "1. Access Jupyter at http://localhost:8888 for GroupBy development and verification"
    echo "2. Use Spark at http://localhost:8080 for batch processing"
    echo "3. Monitor MinIO at http://localhost:9001 for storage"
    echo "4. Check Airflow dashboard at http://localhost:8085 for production-like Spark job submission"
    echo
    ...
    echo "🚀 Production-like: Use Airflow DAGs with compiled JSON configs"
    echo
    ...
    echo "2. Check port conflicts: lsof -i :9000 -i :8080 -i :8888 -i :27017 -i :8085 -i :8000"
...
# Main execution
main() {
    print_status "Starting Chronon Developer setup..."
    ...
    print_success "Chronon Developer setup completed successfully!"
}
