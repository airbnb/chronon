...
class VectorAverage extends SimpleAggregator[Array[Double], Array[Any], Array[Double]] {
  ...
  override def irType: DataType =
    StructType(
      "VectorAverageIr",
      Array(StructField("vector_average", ListType(DoubleType)), StructField("count", IntType))
    )

  override def prepare(input: Array[Double]): Array[Any] = {
    Array(input, 1)
  }
  ...
  override def update(ir: Array[Any], input: Array[Double]): Array[Any] = {
    val previousVectorAverage = ir(0).asInstanceOf[Array[Double]]
    val previousCount = ir(1).asInstanceOf[Int]

    computeRunningAverage(previousVectorAverage, previousCount, input, 1)
  }

  override def merge(ir1: Array[Any], ir2: Array[Any]): Array[Any] = {
    val averages1 = ir1(0).asInstanceOf[Array[Double]]
    val count1 = ir1(1).asInstanceOf[Int]
    val averages2 = ir2(0).asInstanceOf[Array[Double]]
    val count2 = ir2(1).asInstanceOf[Int]

    computeRunningAverage(averages1, count1, averages2, count2)
  }

  override def finalize(ir: Array[Any]): Array[Double] =
    ir(0).asInstanceOf[Array[Double]]

  override def delete(ir: Array[Any], input: Array[Double]): Array[Any] = {
    val previousVectorAverage = ir(0).asInstanceOf[Array[Double]]
    val previousCount = ir(1).asInstanceOf[Int]
