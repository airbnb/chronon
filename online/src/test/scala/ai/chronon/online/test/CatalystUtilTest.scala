...
import ai.chronon.online.DerivationUtils.applyDeriveFunc
import ai.chronon.online.Fetcher.Request
import ai.chronon.online.{CatalystUtil, DerivationUtils, PoolMap, PooledCatalystUtil}
...
class CatalystUtilTest extends TestCase with CatalystUtilTestSparkSQLStructs {
  ...
  def testSelectStarWithCommonScalarsNullShouldReturnNulls(): Unit = {
    ...
    val cu = new CatalystUtil(selects, CommonScalarsStruct)
    val res = cu.sqlTransform(CommonScalarsNullRow)
    assertEquals(res.get.size, 6)
  ...
  def testSelectWithNestedShouldWork(): Unit = {
    ...
    val res = cu.sqlTransform(NestedRow)
    assertEquals(res.get.size, 6)
  ...
  def testSelectWithNestedNullsShouldWork(): Unit = {
    ...
    val cu = new CatalystUtil(selects, NestedOuterStruct)
    val res = cu.sqlTransform(NestedNullRow)
  ...
  def testSelectStarWithListContainersShouldReturnAsIs(): Unit = {
    ...
    val res = cu.sqlTransform(ListContainersRow)
    assertEquals(res.get.size, 6)
  ...
  def testSelectStarWithListArrayContainersShouldReturnAsIs(): Unit = {
    ...
    val cu = new CatalystUtil(selects, ListContainersStruct)
    val res = cu.sqlTransform(ArrayContainersRow)
    assertEquals(res.get.size, 6)
    assertEquals(res.get("bools"), makeArrayList(false, true, false))
    assertEquals(res.get("int32s"), makeArrayList(1, 2, 3))
    assertEquals(res.get("int64s"), makeArrayList(4L, 5L, 6L))
  ...
  def testSelectStarWithMapContainersShouldReturnAsIs(): Unit = {
    ...
    val res = cu.sqlTransform(MapContainersRow)
    assertEquals(res.get.size, 6)
  ...
  def testWhereClauseShouldFilterEventOut(): Unit = {
    ...
    val res = cu.sqlTransform(inputEventRow)
    assertTrue(res.isEmpty)
  }
  ...
  // Regression guard for the Java-25 Janino-codegen failure. When Spark's bundled Janino
  // can't compile generated bytecode on the running JVM, FetcherBase silently catches the
  // exception and returns derivation_fetch_exception / derivation_rename_exception keys
  // in place of features. This test runs the full DerivationUtils path that FetcherBase
  // uses for SQL derivations and asserts the response carries real values, not error keys.
  // If this test fails on a new JVM version, do not catch the exception — the upstream
  // codegen path is broken and consumers will silent-fail in production.

  private val derivationKeySchema: StructType =
    StructType("keys", Array(StructField("user_id", StringType)))

  private val derivationValueSchema: StructType = StructType(
    "values",
    Array(
      StructField("score_raw", DoubleType),
      StructField("count_30d", LongType),
      StructField("status", StringType)
    )
  )

  private val derivationBaseValues: Map[String, AnyRef] = Map(
    "score_raw" -> Double.box(0.42),
    "count_30d" -> Long.box(7L),
    "status" -> "active"
  )

  @Test
  def testSqlDerivationRoundTripsThroughPooledCatalystUtil(): Unit = {
    val derivations = List(
      Builders.Derivation.star(),
      Builders.Derivation(
        name = "score_bucket",
        expression = "CASE WHEN score_raw >= 0.5 THEN 'high' ELSE 'low' END"
      ),
      Builders.Derivation(
        name = "count_or_zero",
        expression = "COALESCE(count_30d, 0)"
      )
    )
    val cu = DerivationUtils.buildCatalystUtil(derivations, derivationKeySchema, derivationValueSchema)
    val keys: Map[String, Any] = Map("user_id" -> "u1")
    val out = cu.applyDerivations(keys ++ derivationBaseValues ++ Map("ts" -> 0L, "ds" -> "2026-01-01")).get
    assertEquals("low", out("score_bucket"))
    assertEquals(7L, out("count_or_zero"))
  }

  @Test
  def testSqlDerivationFetcherPathHasNoExceptionKeys(): Unit = {
    val derivations = List(
      Builders.Derivation.star(),
      Builders.Derivation(
        name = "score_bucket",
    ...
    val deriveFunc =
      DerivationUtils.buildDerivationFunction(derivations, derivationKeySchema, derivationValueSchema)
    val request = Request(name = "regression_test_gb", keys = Map("user_id" -> "u1"))
    val derived = applyDeriveFunc(deriveFunc, request, derivationBaseValues)

    assertFalse(
      s"derivation_fetch_exception present in response: ${derived.get("derivation_fetch_exception")}",
      derived.contains("derivation_fetch_exception"))
    assertFalse(
      s"derivation_rename_exception present in response: ${derived.get("derivation_rename_exception")}",
      derived.contains("derivation_rename_exception"))
    assertEquals("low", derived("score_bucket"))
    assertEquals(7L, derived("count_or_zero"))
    assertEquals("active", derived("status"))
  }
}
