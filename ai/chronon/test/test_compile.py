...
def _invoke_cli_with_params(runner, input_path, flags=None):
    ...
    command = [
        "--chronon_root=ai/chronon/test/sample",
...
def test_basic_compile():
    runner = CliRunner()
    result = runner.invoke(extract_and_convert, ["--chronon_root=ai/chronon/test/sample", "--input_path=joins/sample_team/"])
    assert result.exit_code == 0
    result = runner.invoke(extract_and_convert, ["--chronon_root=ai/chronon/test/sample", "--input_path=joins/sample_team"])
    assert result.exit_code == 0
    result = runner.invoke(
        extract_and_convert, ["--chronon_root=ai/chronon/test/sample", "--input_path=joins/sample_team/sample_join.py"]
    )
    assert result.exit_code == 0
...
def test_debug_compile():
    runner = CliRunner()
    result = runner.invoke(
        extract_and_convert, ["--chronon_root=ai/chronon/test/sample", "--input_path=joins/sample_team/", "--debug"]
    )
    assert result.exit_code == 0
...
def test_detected_dependent_joins_materialized():
    ...
    assert result.exit_code == 0
    expected_message = "Successfully wrote 8 Join objects to ai/chronon/test/sample/production".strip().lower()
    actual_message = str(result.output).strip().lower()
...
def test_detected_dependent_group_bys_materialized():
    ...
    assert result.exit_code == 0
    expected_message = "Successfully wrote 2 GroupBy objects to ai/chronon/test/sample/production".strip().lower()
    actual_message = str(result.output).strip().lower()
    assert expected_message in actual_message, f"Got a different message than expected {actual_message}"
...
def test_detected_dependent_nested_joins():
    ...
    assert result.exit_code == 0
    expected_message = "Successfully wrote 1 Join objects to ai/chronon/test/sample/production".strip().lower()
    actual_message = str(result.output).strip().lower()
    assert expected_message in actual_message, f"Got a different message than expected {actual_message}"
...
def test_compile_dependency_staging_query():
    ...
    result = runner.invoke(
        extract_and_convert,
        ["--chronon_root=ai/chronon/test/sample", f"--input_path={input_path}"],
    )
