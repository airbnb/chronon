  ...
  implicit class JoinOps(val join: Join) extends Serializable {
    ...
    def semanticHash: Map[String, String] = {
      ...
      val derivedHashMap = Option(join.derivations)
        ...
        .getOrElse(Map.empty)
      partHashes ++ Map(leftSourceKey -> leftHash) ++ derivedHashMap
    }
    ...
    def tablesToDrop(oldSemanticHash: Map[String, String]): Seq[String] = {
      ...
      val addedParts = partHashes(newSemanticHash).keys.filter(!oldSemanticHash.contains(_))
      ...
      val mainTable = if (partsToDrop.nonEmpty || addedParts.nonEmpty || derivedChanges) {
