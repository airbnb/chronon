...
class PySparkExecutable(Generic[T], ABC):
    ...
    def _execute_underlying_join_sources(
        ...
        for group_by in group_bys:
            ...
            # Recall that records generated by the inner join are input events
            # for the outer join. Therefore in order to correctly aggregate the
            # outer join, your inner join needs to be run from
            # start_date - max_window_for_gb_in_days
            max_window_for_gb_in_days: int = get_max_window_for_gb_in_days(
                group_by)
            ...
            for js in group_by_join_sources:
                ...
                if js_name is None:
                    raise ValueError(
                        f"Join source {js} does not have a name. "
                        "Was set_metadata called?"
                    )
                ...
                if js_name not in join_sources_to_execute_start_dates:
                    ...
                    existing_start_date: str = join_sources_to_execute_start_dates[
                        js_name
                    ]
                    join_sources_to_execute_start_dates[js_name] = min(
                        shifted_start_date, existing_start_date
                    )
        ...
        self.platform.log_operation(
            f"Executing {len(joins_to_execute)} Join Sources"
        )

        for join in joins_to_execute:
            j_start_date: str = join_sources_to_execute_start_dates[
                join.metaData.name
            ]

            executable_join_cls: type[JoinExecutable] = (
                self.platform.get_executable_join_cls()
            )

            executable_join = executable_join_cls(join, self.spark)

            self.platform.log_operation(
                f"Executing Join Source {join.metaData.name} "
                f"from {j_start_date} to {end_date}"
            )
            _ = executable_join.run(
                start_date=j_start_date, end_date=end_date, step_days=step_days
            )

            output_table_name_for_js: str = output_table_name(
                join, full_name=True
            )
            self.platform.log_operation(
                f"Join Source {join.metaData.name} will be read from "
                f"{output_table_name_for_js}"
            )
    ...
    def group_by_to_java(self, group_by: GroupBy) -> JavaObject:
        ...
        java_group_by: JavaObject = self.jvm.ai.chronon.spark.PySparkUtils.parseGroupBy(
            json_representation
        )
    ...
    def join_to_java(self, join: Join) -> JavaObject:
        ...
        java_join: JavaObject = self.jvm.ai.chronon.spark.PySparkUtils.parseJoin(
            json_representation
        )
        return java_join
...
class GroupByExecutable(PySparkExecutable[GroupBy], ABC):
    ...
    def run(
        ...
        self.platform.log_operation(
            ...
            f"{skip_execution_of_underlying_join}"
        )
        ...
        group_by_output_table: str = output_table_name(
            group_by_to_execute, full_name=True
        )

        # GroupBy backfills don't store the semantic hash as a property in
        # the table the same way joins do.
        ...
        log_token = self.platform.start_log_capture(
            f"Run GroupBy: {self.obj.metaData.name}"
        )
        ...
        try:
            ...
            self.platform.log_operation(
                f"GroupBy {self.obj.metaData.name} executed successfully "
                f"and was written to {group_by_output_table}"
            )
            return result_df
        except Exception as e:
            self.platform.end_log_capture(log_token)
            self.platform.log_operation(
                f"Execution failed for GroupBy {self.obj.metaData.name}: {str(e)}"
            )
            raise e
    ...
    def analyze(
        ...
        self.platform.log_operation(
            f"Analyzing GroupBy {self.obj.metaData.name} from "
        ...
        # Update sources with correct dates
        group_by_to_analyze = self._update_source_dates_for_group_by(
            group_by_to_analyze, start_date, end_date
        )

        # Start log capture just before executing JVM calls
        log_token = self.platform.start_log_capture(
            f"Analyze GroupBy: {self.obj.metaData.name}"
        )
        ...
        try:
            ...
            self.platform.end_log_capture(log_token)
            self.platform.log_operation(
                f"GroupBy {self.obj.metaData.name} analyzed successfully"
            )
        except Exception as e:
            self.platform.end_log_capture(log_token)
            self.platform.log_operation(
...
class JoinExecutable(PySparkExecutable[Join], ABC):
    ...
    def run(
        ...
        self.platform.log_operation(
            f"Executing Join {self.obj.metaData.name} from "
            f"{start_date} to {end_date} with step_days {step_days}"
        )
        ...
        join_to_execute.left = self._update_source_dates(
            join_to_execute.left, start_date, end_date
        )

        if not skip_execution_of_underlying_join and self.obj.joinParts:
            self._execute_underlying_join_sources(
                [jp.groupBy for jp in join_to_execute.joinParts],
        ...
        # Start log capture just before executing JVM calls
        log_token = self.platform.start_log_capture(
            f"Run Join: {self.obj.metaData.name}"
        )
        ...
        try:
            ...
            self.platform.end_log_capture(log_token)
            self.platform.log_operation(
                f"Join {self.obj.metaData.name} executed successfully and "
                f"was written to {join_output_table}"
            )
            return result_df
        except Exception as e:
            self.platform.end_log_capture(log_token)
            self.platform.log_operation(
                f"Execution failed for Join {self.obj.metaData.name}: {str(e)}"
            )
            raise e
    ...
    def analyze(
        ...
        # Start log capture just before executing JVM calls
        log_token = self.platform.start_log_capture(
            f"Analyze Join: {self.obj.metaData.name}"
        )
        ...
        try:
            ...
            self.platform.end_log_capture(log_token)
            self.platform.log_operation(
                f"Join {self.obj.metaData.name} analyzed successfully"
            )
...
class PlatformInterface(ABC):
    ...
    def register_udfs(self) -> None:
        ...
        This method is intentionally left empty but not abstract, as some
        platforms may not need to register UDFs.

        Subclasses can override this method to provide platform-specific UDF
        registration.

        Pro tip: Both the JVM Spark Session and Python Spark Session use the
        same spark-sql engine. You can register Python UDFS and use them in
        the JVM, as well as vice-versa.
        At Stripe we currently only use Scala UDFs, so we include a JAR of our
        UDFs in the cluster and register them via:

        self.jvm.com.stripe.shepherd.udfs.ShepherdUdfRegistry.registerAllUdfs(
        ...
        # Handle nested objects for Join
            ...
            for jp in obj.joinParts:
                for s in jp.groupBy.sources:
                    if s.joinSource and not s.joinSource.join.metaData.name:
                        set_name(
                            obj=s.joinSource.join, cls=Join, mod_prefix=mod_prefix
                        )
