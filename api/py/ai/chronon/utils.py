...
def applicable_mode_table_map(conf: ChrononJobTypes) -> dict:
    ...
    # create empty dict to start mapping modes to tables
    table_map = {}  # type: dict

    # start figure out the applicable modes
    if isinstance(conf, api.GroupBy):
        group_by = cast(api.GroupBy, conf)
        if group_by.backfillStartDate is not None:
            table_map["backfill"] = table_name

        online = group_by.metaData.online or False
        if online:
            table_map["upload"] = f"{table_name}_upload"
            # check if temporal accuracy or streaming is enabled
            temporal_accuracy = group_by.accuracy or False
            streaming = has_topic(group_by)
            if temporal_accuracy or streaming:
                # there are no hive tables created for streaming mode
                table_map["streaming"] = None
    elif isinstance(conf, api.Join):
        join = cast(api.Join, conf)
        if get_offline_schedule(conf) is not None:
            table_map["backfill"] = table_name
            table_map["stats-summary"] = f"{table_name}_daily_stats"
        if (
            join.metaData.customJson is not None
            and json.loads(join.metaData.customJson).get("check_consistency") is True
        ):
            table_map["consistency-metrics-compute"] = f"{table_name}_consistency"
        if requires_log_flattening_task(join):
            table_map["log-flattener"] = f"{table_name}_logged"
        if join.labelPart is not None:
            label_dict = {}
            label_dict["label_table"] = f"{table_name}_labels"
            label_dict["labeled_table"] = f"{table_name}_labeled"
            label_dict["labeled_latest_table"] = f"{table_name}_labeled_latest"
            table_map["label-join"] = label_dict
    ...
    if isinstance(conf, api.Join) and conf.bootstrapParts:
        table_map["bootstrap"] = f"{table_name}_bootstrap"

    return table_map


def get_related_table_names(conf: ChrononJobTypes) -> List[str]:
    """Based on a conf determine if a table will be generated by a task"""

    related_tables = []  # type: List[str]
    mode_table_map = applicable_mode_table_map(conf)
    for mode in mode_table_map:
        if isinstance(mode_table_map[mode], str):
            if mode_table_map[mode] is not None:
                related_tables.append(mode_table_map[mode])
        elif isinstance(mode_table_map[mode], dict):
            for _, v in mode_table_map[mode].items():
                related_tables.append(v)

    return related_tables


def get_applicable_modes(conf: ChrononJobTypes) -> List[str]:
    """Based on a conf determine if a mode of task will be run."""

    mode_table_map = applicable_mode_table_map(conf)
    list_modes = [m for m in mode_table_map.keys() if m != "bootstrap"]
    return list_modes
