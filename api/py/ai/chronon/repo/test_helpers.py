...
def create_mock_group_by(gb: GroupBy, input_df: DataFrame, spark: SparkSession) -> JavaClass:
    ...
    java_spark_group_by: JavaClass = jvm.ai.chronon.spark.PySparkUtils.buildGroupByUsingArrayList(
...
def create_mock_source(source: Source, accuracy: Accuracy, key_columns: List[str], mock_underlying_table_df: DataFrame, spark: SparkSession): 
    ...
    source_query: str = jvm.ai.chronon.spark.PySparkUtils.renderUnpartitionedDataSourceQueryWithArrayList(
        java_thrift_source,
        key_columns,
